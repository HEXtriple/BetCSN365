...
function getSlotMachineSpin(rows: number, cols: number, symbols: { [key: string]: number }): string[][] {
...
function getNumberOfLines(): number {
  while (true) {
    const linesStr = prompt(`Enter the number of lines to bet on (1-${MAX_LINES}): `);
...
function main(): void {
  ...
  while (true) {
    ...
    const { winnings, winningLines } = checkWinnings(slots, lines, bet, symbolValue);
